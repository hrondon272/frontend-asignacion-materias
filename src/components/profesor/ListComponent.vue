<template>
    ...
    <router-link :to="{ name: 'saveProfesor' }" style="text-decoration: none;">
        <o-button variant="info">
            Registrar profesor
        </o-button>
    </router-link>

    <o-table :data="profesores.length == 0 ? []: profesores">
        ...
        <o-table-column Field="accion" label="ACCIÓN" string v-slot="p">
            <div class="odocs-spaced">
                <router-link :to="{ name: 'editProfesor', params: { profesorId: p.row.id } }" style="text-decoration: none;">
                ...
                <router-link :to="{ name: 'asignaturasProfesor', params: { profesorId: p.row.id } }" style="text-decoration: none;">
                    <o-button variant="info">
                        Ver asignaturas
                    </o-button>
                </router-link>
...
<script>
    export default{
        ...
        async mounted() {
            var config = {
                method: 'get',
                url: 'http://127.0.0.1:8000/api/profesor',
                headers: { 
        ...
        methods: {
            eliminar(id){
                ...
                if (confirm("Por favor confirme para continuar")) {
                    var config = {
                    method: 'delete',
                    url: 'http://127.0.0.1:8000/api/profesor/'+id+'',
                    headers: { 
                        'APP_KEY': 'Ke9HAnknaQfYo%EPa7sjG^G3jknZ9ThFANZXgV$t4Nvca%XDy@35VWhwKSP37pTAm4F6CVDHL7$c3v3qBWM4hg7Kx@tsW$pTe9U726UyD&2njqtKnaSHbw9C', 
                        //'Authorization': 'Bearer 1|XP6X8BviZngrXWu8GkEEFV4znR8wio3gu9JZOiTg', 
                        'Content-Type': 'application/json'
